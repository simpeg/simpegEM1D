...
source_heights = mkvc(source_locations[:, -1])
receiver_offsets = np.c_[source_locations[:, 0]+10., source_locations[:, 1], np.zeros(np.size(source_heights))]
...
for ii in range(0, n_sounding):
    ...
    receiver_offset = mkvc(receiver_offsets[ii, :])
    
    receiver_list = []
    
    receiver_list.append(
        em1d.receivers.HarmonicPointReceiver(
            receiver_offset, frequencies, orientation=receiver_orientation,
...
wires = maps.Wires(('sigma', mesh.nC),('height', n_sounding))
sigma_map = maps.ExpMap(mesh) * wires.sigma
height_map = wires.height

starting_model = np.r_[
    np.log(conductivity.reshape(mesh.vnC, order='F').flatten()),
    25*np.ones(n_sounding)
]
...
simulation = em1d.simulation_stitched1d.GlobalEM1DSimulationFD(
    mesh, survey=survey, sigmaMap=sigma_map, hMap=height_map, hz=hz, topo=topo,
...
reg_lateral = LateralConstraint(
    mesh_reg, mapping=maps.IdentityMap(nP=mesh.nC),
    alpha_s = 0.1,
    alpha_x = 0.0001,
    alpha_y = 1.,
)
xy = utils.ndgrid(np.arange(n_sounding), np.r_[0.])
reg_lateral.get_grad_horizontal(xy, hz, dim=2, use_cell_weights=True)
...
ps, px, py = 1, 1, 1
reg_sigma.norms = np.c_[ps, px, py, 0]


mesh_height = TensorMesh([dx*np.ones(n_sounding)])
reg_height = regularization.Sparse(mesh_height, mapping=wires.height)
