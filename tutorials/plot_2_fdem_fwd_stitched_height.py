...
# model = model.reshape(mesh.vnC, order='F').flatten()
...
model = np.r_[np.log(model), source_heights]

wires = maps.Wires(('sigma', mesh.nC),('h', n_sounding))
...
simulation = em1d.simulation_stitched1d.GlobalEM1DSimulationFD(
    mesh, survey=survey, sigmaMap=sigma_map, hMap=h_map, chi=chi, hz=hz, topo=topo,
