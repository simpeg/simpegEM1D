...
class BaseEM1DSurvey(Survey.BaseSurvey, properties.HasProperties):
    ...
    def n_layer(self):
        """
            Srource height
        """
        if self.half_switch == False:
            return self.depth.size
        elif self.half_switch == True:
            return int(1)
...
class EM1DSurveyTD(BaseEM1DSurvey):
    ...
    def setFrequency(self, time=np.logspace(-7, -1, 256)):
        ...
        if self.offset is not None and np.isscalar(self.offset):
            self.offset = self.offset*np.ones(self.n_freq)
        elif self.offset is not None and not np.isscalar(self.offset):
            self.offset = self.offset[0]*np.ones(self.n_freq)
